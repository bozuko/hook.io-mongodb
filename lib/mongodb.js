...
var Mongodb = module.exports = function (options) {
  ...
  self.host = self.config.get('host') || defaults.host;
  self.port = self.config.get('port') || defaults.load_threshold;
  self.db_name = self.config.get('db_name') || defaults.db_name;
  self.rs_name = self.config.get('rs_name');
  self.poll_time = self.config.get('poll_time') || defaults.poll_time;
  self.timeout = self.config.get('timeout') || defaults.timeout;
  self.threshold = merge(defaults.threshold, self.config.get('threshold') || {});
  self.setup();
